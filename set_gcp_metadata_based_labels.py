...
class SetMetadatabasedLabels:
    ...
    def main(self, organization_id):
        """Runs the full Class Functionality to Set Labels based on Project / Org Metadata. 
        
        Args:
            organization_id (str): numerical Organization ID 
        """
        try:
            if organization_id == "0":
                get_projects_class = GetGCPFoldersAndProjects(organization_id) # Instanciate GetGCPFoldersAndProjects Class. 
            ...
            for project_id in get_projects:
                ...
                if resources and "assets" in resources and resources["assets"]:
                    for asset in resources["assets"]:
                        ...
                        # The idea here is to add Resource Type Labeling setting functionality. It must be resource by resource because each as to use the respective API SDK. 
                        if asset_type == "Instance":
                            self.set_instance_labels(project_id, asset_zone, asset_name, resultant_labels)
                            
                    ## This is for result visualization 
                    resources = self.fetch_resources(project_id)
                    for asset in resources["assets"]:
                        current_label_status = asset.get('resource', {}).get('data', {}).get('labels')
                else:
    ...
    def fetch_resources(self, project_id):
        """
        This function is used to fetch resources from the Assets API in GCP. 
        Its important to have into account that this functions receive the information in gRPC so it needs to be using the ProtoBuf format. 

        Args:
            project_id (str): Project ID of the GCP Project where to fetch the resources. 

        Returns:
            list(): return the list of resources 
        """
        try:
    ...
    def merge_labels(self, metadata_labels, current_labels):
        """
        Function to merge the existing labels of the VM with the New ones. 
        Important limitations: 
            * Compute Engine only supports 64 total Labels. 
            * If the resource exceed this number empty and excess labels will be deleted:
                1. Remove "safe to delete" labels, if exists. 
                2. Remove empty labels (keys with no value).
                3. Remove the exceeded values when sorted. 

        Args:
            metadata_labels (dict()): Labels calculated by the script. 
            current_labels (dict()): Current Labels of the resource. 

        Returns:
            dict(): Resultant labels 
        """
        ...
        for key in safe_to_remove_keys:
            resultant_labels.pop(key, None)  
               
        if len(resultant_labels) > 64:
            ...
            excess_count = len(resultant_labels) - 64
    ...
    def create_metadata_labels(self, asset, project_id, asset_type, asset_name, asset_zone):
        ...
        """This Function create the metadata labels based on the Asset information. 

        Args:
            asset (json): The Json extracted in the Fetch Resources function.
            project_id (str): GCP Project ID 
            asset_type (str): Extracted in the Fetch Resources function
            asset_name (str): Extracted in the Fetch Resources function
            asset_zone (str): Extracted in the Fetch Resources function

        Returns:
            dict(): Metadata Labels resultant dictionary
        """
        ...
        if asset_os == 'unknown':
            for disk in disks:
                ...
                for license_url in licenses:
                    license_os = license_url.split('/')[-1].lower() 
    ...
    def set_instance_labels(self, project_id, zone, instance_name, labels):
        """This functions finally set the new labels to the Compute Instance. 

        Args:
            project_id (str): GCP Project ID. 
            zone (str): GCP Zone. 
            instance_name (str): Instance Name. 
            labels (dict()): Labels key:value pairs. 
        ...
        try:
            ...
            operation = compute_client.set_labels(request=label_request) # Exectue Operation 
